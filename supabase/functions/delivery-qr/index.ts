...
serve(async (req) => {
  ...
  try {
    ...
    const { action, order_id, buyer_id, seller_id, token } = await req.json();
    ...
    if (action === 'validate') {
      ...
      const logScan = async (result: string, errorMessage?: string, qrId?: string, orderId?: string) => {
        await supabase.from('qr_scan_logs').insert({
          ...
          metadata: { token_prefix: token.substring(0, 20) }
        });
      };
      ...
      const earningReference = `qr-escrow-release:${order.id}`;
      ...
      if (existingEarning) {
        ...
        await logScan('success', 'Already processed (idempotent)', qrCode.id, qrCode.order_id);
        return new Response(
          JSON.stringify({ 
            success: true, 
      ...
      await supabase.from('notifications').insert({
        ...
        body: `QR verified for "${order.products?.title}". Le ${amountToRelease.toLocaleString()} added to your wallet.`,
      ...
      // Delete QR code after successful scan
      await supabase
        .from('delivery_qr_codes')
        .delete()
        .eq('id', qrCode.id);
    ...
    if (action === 'status') {
      ...
      if (error || !qrCode) {
        return new Response(
          JSON.stringify({ has_active_qr: false }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
      ...
      if (isExpired) {
        ...
        return new Response(
          JSON.stringify({ has_active_qr: false, was_expired: true }),
          { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        );
      }
