  ...
  const handleDeleteMessage = async (forEveryone: boolean) => {
    ...
    try {
      if (forEveryone) {
        ...
        // Delete for me - just mark as deleted locally (you could add a deleted_for field)
        setMessages(prev => prev.filter(m => m.id !== selectedMessage.id));
      ...
      >
        ...
        <div className={`flex flex-col ${isOwn ? 'items-end' : 'items-start'} max-w-[75%]`}>
          ...
          {replyToMessage?.id === message.id && (
            <div className="mb-1 px-3 py-1 bg-muted/50 rounded-lg text-xs text-muted-foreground">
          ...
          <div
            className={`px-4 py-3 rounded-2xl shadow-sm transition-all cursor-pointer hover:shadow-md ${
              isOwn
